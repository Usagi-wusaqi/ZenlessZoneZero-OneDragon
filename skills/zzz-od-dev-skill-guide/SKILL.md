...
### 3. 自包含:可引 skill,不引文件
- **可引用其它 skill**(写**完整标识符含命名空间**,如「按 superpowers:receiving-code-review 的方法」):skill 可独立发布,使用者同时具备即可。用完整标识符,避免裸名解析不到。
- **禁止引用 skill 目录外的文件**(如「详见 docs/xxx.md」「参考 ../../xxx.py」):发布不含这些,目标智能体环境里可能不存在。
- 要用的知识直接内联进 SKILL.md 或 skill 目录内辅助文件,不外指。

### 4. 写方法论,不写具体例子(限 SKILL.md 与智能体读的辅助文件)
写**方法、原则、判据**(怎么判断、怎么选),不要写「某个具体场景/项目的做法」。
