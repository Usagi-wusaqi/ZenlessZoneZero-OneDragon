...
脚本环境配置 `force_active_window` 默认关闭，可在 `设置 → 脚本环境 → 基础` 中开启。该配置只作用于前台模式：控制器在获取截图前发现游戏窗口失去焦点时，会尝试恢复游戏窗口，再继续当前操作；后台模式不会因此抢占前台。

开启后，窗口激活逻辑会持续确认游戏窗口是否真正位于前台。多次尝试仍未成功时，会通过任务栏命令尝试最小化其他窗口；如果最终仍无法确认焦点，截图操作会抛出异常并停止当前操作，避免误点其他窗口。设置项变化时会自动重新初始化控制器，运行任务时切出游戏前应先暂停任务。
...
- `active_window()` — 后台模式直接返回成功；前台模式尝试恢复游戏窗口并返回是否已确认获得焦点
