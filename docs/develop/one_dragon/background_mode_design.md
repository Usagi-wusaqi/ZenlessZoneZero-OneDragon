...
游戏在失去焦点后会忽略 `WM_LBUTTONDOWN`/`WM_LBUTTONUP` 消息。后台点击会先将物理光标移到目标位置，再发送
`WM_ACTIVATE(WA_ACTIVE)`，让游戏认为自己处于激活状态，后续的点击消息即可被处理。

**消息序列**：

```
1. SetCursorPos(ClientToScreen(hwnd, (x, y)))  -- 先移动物理光标
2. sleep(10ms)
3. SendMessage(hwnd, WM_ACTIVATE, WA_ACTIVE, 0)   -- 假装激活
4. sleep(10ms)
5. PostMessage(hwnd, WM_LBUTTONDOWN, MK_LBUTTON, 0)
6. sleep(20ms)
7. PostMessage(hwnd, WM_LBUTTONUP, 0, 0)
```
...
# 'compendium' → ['xbox_lt', 'xbox_a']   (LT+A)
...
_KEY_DEFAULTS: dict[str, dict[str, str]] = {
    'key':      {'interact': 'f', 'dodge': 'shift', ...},      # 键盘 (16 个)
...
_ACTION_KEY_DEFAULTS: dict[str, dict[str, list[str]]] = {
    'xbox_action': {'menu': ['xbox_start'], 'compendium': ['xbox_lt', 'xbox_a'], ...},
    'ds4_action':  {'menu': ['ds4_options'], 'compendium': ['ds4_l2', 'ds4_cross'], ...},
}
```
...
for prefix, defaults in _KEY_DEFAULTS.items():
    for action in GameKeyAction:          # 16 个动作
...
```
PcControllerBase
├── background_mode: bool        → 当前控制器是否使用后台模式
...
脚本环境配置 `force_active_window` 默认关闭，可在 `设置 → 脚本环境 → 基础` 中通过“运行时恢复游戏窗口焦点”开启。该配置只作用于前台模式的运行期焦点保护：控制器在获取截图前发现游戏窗口失去焦点时，会尝试恢复游戏窗口，再继续当前操作；后台模式不会因此抢占前台。

开启后，窗口激活逻辑最多尝试 30 次确认游戏窗口是否真正位于前台。每 10 次仍未成功时，会通过任务栏命令尝试最小化其他窗口；如果最终仍无法确认焦点，截图操作会抛出异常并停止当前操作，避免误点其他窗口。设置项变化时会自动重新初始化控制器，运行任务时切出游戏前应先暂停任务。

启动阶段与运行阶段使用不同入口：游戏启动或窗口刚就绪时调用 `active_window()`，只尝试一次，不读取 `force_active_window`，也不会最小化其他窗口；运行期截图失焦时调用 `ensure_active_window()`，才会按配置启用持续恢复和失败兜底。

### `ZPcController.init_before_context_run()` — 配置刷新

每次 `start_running()` 时自动刷新运行所需的配置快照，保证设置界面的修改在下次运行生效：
...
所有按键映射（如 `{'dodge': 'shift', 'interact': 'f', ...}`），由 `GameConfig.get_action_keys(control_method)`
根据控制方式返回。初始化时始终加载键盘键名，`enable_xbox()`/`enable_ds4()` 切换时同步更新。

**动作方法**：16 个动作方法（`dodge()`、`normal_attack()` 等）均为一行委托：
...
1. `SetForegroundWindow(hwnd)` 切到前台
...
| 截图 | 不受影响（已有后台截图） | 默认不检查焦点，可选前台焦点保护 |
...
- `background_mode: bool` — 当前控制器是否使用后台模式
- `force_active_window: bool` — 前台模式截图前是否强制恢复游戏窗口焦点
- `active_window()` — 后台模式直接返回成功；前台模式只尝试一次恢复窗口，不读取运行期强制恢复配置
- `ensure_active_window()` — 运行期截图失焦恢复入口，根据 `force_active_window` 决定是否持续重试及启用最小化其他窗口兜底
...
- `active(retry_until_active=False)` — 尝试恢复窗口并确认前台焦点；开启持续重试后，每失败 10 次尝试最小化其他窗口
- `is_win_active` — 通过前台窗口句柄判断当前游戏窗口是否激活
...
- `get_action_keys(control_method) -> dict[str, str]` — 返回 `{action_name: key_value}`
  - `control_method`：`'keyboard'` / `'xbox'` / `'ds4'`，需要显式传入
...
- `get_gamepad_action_keys(gamepad_type) -> dict[str, list[str]]` — 返回 `{action_name: [key, ...]}`
  ...
  - 例如 `get_gamepad_action_keys('xbox')` → `{'menu': ['xbox_start'], 'compendium': ['xbox_lt', 'xbox_a'], ...}`

## 前置条件

1. **ViGEmBus 驱动**：需要安装（安装器可集成）
2. **vgamepad Python 包**：项目运行依赖已声明，使用 `uv sync` 安装
