...
- **back_to_world 方法**: 继承自 `ZApplication` 的应用类应遵循以下规范：
  - **优先使用默认实现**: 大多数应用无需重写 `back_to_world` 方法，直接使用基类提供的默认实现。
  - **自定义状态消息**: 需要自定义状态消息时，使用 `custom_status` 参数：`return super().back_to_world(custom_status="自定义状态")`
  - **特殊逻辑处理**: 只有在需要特殊业务逻辑时才完全重写该方法。
  - **文档说明**: 重写该方法时必须在注释中说明重写的原因和特殊逻辑。
  - **详细使用指南**: 参考 [back_to_world 使用指南](../zzz/application/back_to_world_usage.md)
